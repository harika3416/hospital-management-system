...
public class PatientController {
	...
	public ResponseEntity<Patient> getPatient(@PathVariable int id){
	 ...
	 @PostMapping("/patients")
		public ResponseEntity<?> addProduct(@RequestPart Patient patient,@RequestPart MultipartFile imageFile){
			 try {
				 Patient patient1=service.addPatient(patient,imageFile);
	 ...
	 public ResponseEntity<byte[]> getImageByPatientId(@PathVariable int patientId){
		 Patient patient=service.getPatientById(patientId);
		 byte[] imageFile=patient.getImageData();
		  
		 return ResponseEntity.ok()
				 .contentType(MediaType.valueOf(patient.getImageType()))
				 .body(imageFile);
	 }
	 
	 
	 @PutMapping("/patients/{id}")
	 public ResponseEntity<String> updatePatient(@PathVariable int id,@RequestPart Patient patient) throws IOException{
		  Patient patient1=null;
		 try {
		 patient1=service.updatePatient(id,patient);
		 }catch (Exception e) {
